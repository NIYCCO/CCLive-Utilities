...
/*
    EmojiRegistry von ChatGPT geschrieben...
    Hat bei mir gekracht als ich das versucht habe selbst zu coden xD
*/
                ...
                for (var entry : definitions.entrySet()) {
                    ...
                    try (InputStreamReader reader =
                        ...
                        for (JsonElement el : providers) {
                            ...
                            // Optional noch enger:
                            // if (!"custom/chat/emojies.png".equals(getString(prov, "file"))) continue;

                            var chars = prov.getAsJsonArray("chars");
                            if (chars == null) continue;

                            int idx = 0; // Index in EMOJI_NAMES über alle rows hinweg
                            for (JsonElement rowEl : chars) {
                                if (!rowEl.isJsonPrimitive()) continue;

                                // Kein Stream, damit wir sauber abbrechen können
                            ...
                            // Wir haben unseren Provider verarbeitet – weitere default.json ignorieren
                            done = true;
                            break;
                        }
                    } catch (Exception e) {
