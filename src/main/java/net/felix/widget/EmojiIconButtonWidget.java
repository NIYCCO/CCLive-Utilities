...
public class EmojiIconButtonWidget extends TextIconButtonWidget.IconOnly {
    private boolean outlined = false;
    private int buttonColor = 0xA0000000;
    private int buttonColorActive = 0xA0202020;

    public EmojiIconButtonWidget(int width, int height, Text message, int textureWidth, int textureHeight, Identifier texture, PressAction pressAction, @Nullable ButtonWidget.NarrationSupplier narrationSupplier) {
    ...
    public void renderWidget(DrawContext context, int mouseX, int mouseY, float delta){
        ...
        int textColor = this.isHovered() ? 0xFFFFFFFF : 0xFFA0A0A0;
        if(texture != null) {
    ...
    public interface PressAction extends ButtonWidget.PressAction {
        void onPress(EmojiIconButtonWidget button);
        @Override
        default void onPress(ButtonWidget button) {
            if(button instanceof EmojiIconButtonWidget emojiIconButtonWidget) this.onPress(emojiIconButtonWidget);
        }
    }
}
